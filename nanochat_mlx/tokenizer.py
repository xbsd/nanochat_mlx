...
Ported from nanochat/tokenizer.py - framework-agnostic (no PyTorch dependency).

Two implementations are available:
1) HuggingFace Tokenizer that can do both training and inference
2) RustBPE Tokenizer for training and tiktoken for efficient inference
"""
...
class RustBPETokenizer:
    ...
    @classmethod
    def train_from_iterator(cls, text_iterator, vocab_size):
        assert HAS_RUSTBPE, "rustbpe is required for training. Install with: pip install rustbpe"
    ...
    def render_conversation(self, conversation, max_tokens=2048):
        """Tokenize a single Chat conversation. Returns (ids, mask) lists."""
        ids, mask = [], []
    ...
    def render_for_completion(self, conversation):
        """Used during RL - render conversation priming the Assistant for completion."""
        conversation = copy.deepcopy(conversation)
        messages = conversation["messages"]
